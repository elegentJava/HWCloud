...
/**
 * //校验按钮 function verifyButton(obj){ //获得form中包含的input标签 var inputs =
 * obj.tags("input");
 * 
 * //循环遍历input对象数组，取得每一对象，然后调用校验方法（按钮(BUTTON)的校验方法） for(var i=0;i<inputs.length;i++) {
 * //获得input对象 var inputTemp = inputs[i]; var inputType =
 * inputTemp.getAttribute('type') inputType = inputType.toLowerCase();
 * if(inputType=='button') { disableButton(inputTemp); } } }
 */
// 执行校验函数
/*
 * function _checkFile(obj) { //获得校验类型 var verifyType =
 * obj.getAttribute("fileType");//.value; //获得域名称 var verifyName=
 * obj.getAttribute("fileName");//obj.fileName.value; try {
 * //如果校验类型为空或没有书写校验属性，则不对其进行校验 if(verifyType!=null) { var functionName =
 * verifyType+'Verify(obj);'; resultStr = eval(functionName); //校验失败
 * if(resultStr!=success) { appendShowMessage('['+verifyName+']'+resultStr);
 * }else { //校验通过，不执行任何操作 } } }catch(e) {
 * appendShowMessage('校验错误，输入的校验对象为空，请检查代码书写是否正确，注意字符大小写！'); return false; } }
 */
// 执行校验函数,校验文本区域
function _checkFile(obj) {
	...
	try {
		...
		if (verifyLength != null && !isNaN(parseInt(verifyLength))
				&& parseInt(verifyLength) > 0) {
